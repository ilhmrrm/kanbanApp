...
func (u *userAPI) Login(w http.ResponseWriter, r *http.Request) {
	...
	id, err := u.userService.Login(r.Context(), &res)
	...
	json.NewEncoder(w).Encode(map[string]interface{}{"user_id": id, "message": "login success"})
...
func (u *userAPI) Register(w http.ResponseWriter, r *http.Request) {
	...
	userRegis, err := u.userService.Register(r.Context(), &res)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(entity.ErrorResponse{Error: "error internal server"})
		return
	...
	json.NewEncoder(w).Encode(map[string]interface{}{"user_id": userRegis.ID, "message": "register success"})
	// TODO: answer here
}
